            ...
            <div class="messages-section">
                <MTabs>
                    <MTabItem :selected="true" :name="$t('notifications')">
                                      <div class="empties" v-if="Object.keys(notifications).length == []">
                <p class="text-accent-1">{{$t('emptyNotifications')}}</p>
                        ...
                        <div class="notifications" v-else-if="Object.keys(notifications).length > 0">
                            ...
                            <div class="col icon-wrapper">
                                <div class="ic ic_bell" :class="{'ic_bell_active':item.type}"></div>
                            </div>
                            <div class="col-12">
                                <div class="notification-item-row">
                                    <p class="notification-item-message" :class="{'notification-item-message--active':item.type}">{{item.content}}</p>
                                    <p class="text-accent-1 notification-item-date">{{item.createdAt | moment("DD.MM.YYYY")}}</p>
                                </div>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="notifications-mobile" v-else>
                            <div class="notification-item" v-for="(item,index) in notifications.slice(0,12)" :key="index">
                                ...
                                <div class="ic ic_bell" :class="{'ic_bell_active':item.new}"></div>
                                    ...
                                    <p class="text-accent-1">{{ item.createdAt | moment("dddd, MMMM Do YYYY, h:mm:ss a") }}</p>
                                </div>
                                <div class="row">
                                    <p class="notification-item-message" :class="{'notification-item-message--active':item.type}">{{item.content}}</p>
                                </div>
                ...
                <MButton :btnClass="'btn btn-regular app-button'" @click="toCreate">{{$t('sendMsg')}}</MButton>
 <div class="empties" v-if="Object.keys(messages).length == []">
                <p class="text-accent-1">{{$t('emptyMessages')}}</p>
              </div>
                    <div class="app-table-wrapper" v-else-if="Object.keys(messages).length > 0">
                        <table class="table">
                            <thead>
                                <th v-for="(item,index) in messages" :key="index">{{$t(item.title)}}</th>
                            </thead>
                            <tbody>
                                <router-link tag="tr" :to="{name: 'MessageItem', params:{id: item.id}, props:{id:item.id}}" v-for="(item,index) in messages" :key="index">
                                    <td>{{item.createdAt | moment("DD.MM.YYYY")}}</td>
                                    <td>{{item.chatTitle}}</td>
                                    <td>{{item.type == 1 ? $t('open') : item.type == 2 ? $t('closed') : "" }}</td>
                                    <td>{{item.id}}</td>
                                </router-link>
                            </tbody>
                        </table>
                    ...
                    <div class="table-mobile" v-else>
                        <router-link tag="div" class="table-mobile-item" :to="{name: 'MessageItem', params:{id: item.id}, props:{id:item.id}}" v-for="(item,index) in messages" :key="index">
                            <div class="col-6">
                                <div class="table-mobile-item--title">{{$t('createdAt')}}</div>
                                <div class="table-mobile-item--title">{{$t('topic')}}</div>
                                <div class="table-mobile-item--title">{{$t('status')}}</div>
                                <div class="table-mobile-item--title">ID: </div>
                            </div>
                            <div class="col-6">
                                 <div class="table-mobile-item--body">{{item.createdAt | moment("dddd, MMMM Do YYYY, h:mm:ss a")}}</div>
                                <div class="table-mobile-item--body">{{item.chatTitle}}</div>
                                <div class="table-mobile-item--body">{{item.type == 1 ? $t('open') : item.type == 0 ? $t('closed') : ""}}</div>
